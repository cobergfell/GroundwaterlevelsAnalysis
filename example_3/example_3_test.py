...
mydir = os.path.join(path_to_parent_folder,'example_3')         
if not os.path.isdir(mydir):
    os.mkdir(mydir)  
    
filepath = os.path.join(mydir,'model_definition.json')

md = md.from_jason(filepath=filepath) # case 3: model definition specified in json json file in filepath
...
potimizer = Poptimizer(heads = heads, time = time, p_dict = p_dict, time_step = time_step,time_step_targets = time_step_targets,
                        ...
                        Nint_dict = Nint_dict, settings = settings,analytical_jacobian = False)    
...
print('line 173 check output_string: ',output_string)   
